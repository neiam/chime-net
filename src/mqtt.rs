...
impl MqttClient {
    ...
    pub async fn publish_json<T: serde::Serialize + ?Sized>(&self, topic: &str, payload: &T, qos: i32, retain: bool) -> Result<()> {
...
impl ChimeNetMqtt {
    ...
    pub async fn publish_chime_ring(&self, chime_id: &str, ring_request: &ChimeRingRequest) -> Result<()> {
        let topic = TopicBuilder::chime_ring(&self.user, chime_id);
        self.client.publish_json(&topic, ring_request, 1, false).await
    }
    
    pub async fn publish_chime_ring_to_user(&self, user: &str, chime_id: &str, ring_request: &ChimeRingRequest) -> Result<()> {
        let topic = TopicBuilder::chime_ring(user, chime_id);
        self.client.publish_json(&topic, ring_request, 1, false).await
    }
    
    pub async fn publish_chime_response(&self, chime_id: &str, response: &ChimeResponseMessage) -> Result<()> {
